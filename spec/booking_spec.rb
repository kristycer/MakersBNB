...
describe 'booking' do
  ...
  booking = Booking.create(property_name: 'Test Property', booking_date: '2020-10-03' , total_price: 5.00, name_of_booking: 'User Example')

  expect(booking.property_name).to eq 'Test Property' 
  expect(booking.booking_date).to eq '2020-10-03'
  expect(booking.name_of_booking).to eq 'User Example'
  end
